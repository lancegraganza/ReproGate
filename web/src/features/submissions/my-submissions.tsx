...
export function MySubmissions() {
  ...
  return <div className="task-list">{items.map((item) => <Link className="task-card" href={`/app/tasks/${item.taskId}`} key={item.id}><div><span className={`verdict ${item.verdict === "REPRODUCED" ? "reproduced" : "not-reproduced"}`}>{item.verdict.replaceAll("_", " ")}</span><h2>{item.environment.runtime} {item.environment.runtimeVersion} · {item.environment.operatingSystem}</h2><p className="mono field-hint">evidence {item.evidenceHash.slice(0, 18)}…</p>{item.suspiciousReason ? <p className="notice notice-warning">Excluded: {item.suspiciousReason}</p> : null}</div><div className="task-reward"><span className={`status-badge ${item.eligible ? "status-open" : "status-funding"}`}>{item.eligible ? "Eligible" : "Flagged"}</span></div></Link>)}</div>;
}

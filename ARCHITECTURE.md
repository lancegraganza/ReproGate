...
`GET|POST /api/cron/reproduce` is a protected endpoint hosted by Vercel and scheduled externally by cron-job.org every 30 minutes. It returns `202 Accepted` and continues in Next's background `after()` phase, where it acquires a distributed task lock and records an idempotent window run, creates an ephemeral Friendbot-funded Testnet wallet, generates schema-validated synthetic evidence through Gemini, submits it through the same wallet challenge and repository service as the browser evidence form, inserts it as chain-pending, sends a real Testnet XLM payment, and waits for Horizon confirmation. Evidence wording and traces vary, while the normalized environment remains stable enough for independent confirmations to reach the configured threshold. Once the threshold is reached, the repository atomically reserves `FINALIZING` only when no evidence transaction remains pending, freezing the accepted wallets and result hash. A server-only Testnet maintainer signer then prepares and durably checkpoints the exact signed envelope and hash before RPC submission, so recovery safely resubmits the same transaction rather than constructing a duplicate; an expired non-included envelope is replaced, and confirmed signed XDR is erased. `ReproTaskRegistry.finalize` calls `RewardVault.distribute`, pays every accepted wallet through the native XLM SAC, and is accepted only after RPC success, matching Registry events/state, completed vault state, and per-contributor paid reads and events. Runs persist `SUBMISSION_PENDING`, `AWAITING_FINALIZATION`, `FINALIZATION_PENDING`, and form checkpoints. Every Google Form payload is held until Soroban settlement and all payouts are verified. Definite HTTP rejection is retryable; uncertain form delivery is retained for audit and never blindly duplicated. Contributor secrets are ephemeral; the dedicated maintainer seed never leaves the server or enters persistence. Neither endpoint is registered as a Vercel cron.

...
`GET|POST /api/stellar/sync` polls both contracts with a durable cursor behind `CRON_SECRET`; cron-job.org invokes it every five minutes. Event IDs are inserted idempotently, then full typed contract state is compared with off-chain task metadata before any lifecycle transition. Confirmed transaction callbacks additionally require the expected contract event, task hash, transaction hash, and ledger.

`GET|POST /api/cron/reproduce` is a protected endpoint hosted by Vercel and scheduled externally by cron-job.org every 30 minutes. It runs the full workflow synchronously within the request handler (`maxDuration = 300`), acquires a distributed task lock and records an idempotent window run, creates an ephemeral Friendbot-funded Testnet wallet, generates schema-validated synthetic evidence through Gemini, submits it through the same wallet challenge and repository service as the browser evidence form, inserts it as chain-pending, sends a real Testnet XLM payment, and waits for Horizon confirmation. Evidence wording and traces vary, while the normalized environment remains stable enough for independent confirmations to reach the configured threshold. Once the threshold is reached, the repository atomically reserves `FINALIZING` only when no evidence transaction remains pending, freezing the accepted wallets and result hash. A server-only Testnet maintainer signer then prepares and durably checkpoints the exact signed envelope and hash before RPC submission, so recovery safely resubmits the same transaction rather than constructing a duplicate; an expired non-included envelope is replaced, and confirmed signed XDR is erased. `ReproTaskRegistry.finalize` calls `RewardVault.distribute`, pays every accepted wallet through the native XLM SAC, and is accepted only after RPC success, matching Registry events/state, completed vault state, and per-contributor paid reads and events. Each stage persists a granular status (`STARTED → EVIDENCE_SUBMITTED → PAYMENT_CONFIRMED → FINALIZATION_PENDING → FINALIZING → FINALIZED → FORM_PENDING → FORM_SUBMITTED → COMPLETED`) so a crash at any point produces a recoverable state. Google Form payloads are held until Soroban settlement and all payouts are verified. Definite HTTP rejection is retryable; uncertain form delivery is retained for audit and never blindly duplicated. Contributor secrets are ephemeral; the dedicated maintainer seed never leaves the server or enters persistence.

`GET|POST /api/cron/settle` is a lightweight protected endpoint scheduled every two minutes via cron-job.org. It finds automation runs stuck in finalization or form-delivery states and completes them without creating wallets, evidence, or new reproductions. It uses a separate lock key from the reproduce endpoint. This prevents a threshold-triggering run from remaining stuck for 30 minutes waiting for the next reproduction cycle. Neither endpoint is registered as a Vercel cron.
